...
from config import PURPLE, DARK, WHITE, DARKER, GOLD, icon
from api import API
...
class ChatsScreen(Screen):
    ...
    def open_menu(self, instance):
        ...
        items = [('user.png', 'Профиль', 'profile'), ('search.png', 'Контакты', 'search'), ('settings.png', 'Настройки', 'settings')]
        for icon_name, text, screen in items:
            ...
            row.add_widget(lbl)
            row.bind(on_touch_down=lambda i, t, s=screen: self.menu_action(s) if i.collide_point(*t.pos) else None)
